...
static void usage(const char *command)
{
    LOGE("\nUsage:\n"
        ...
        "  -n --netns=N         Set up new a net namespace every N tests\n"
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        switch (arg) {
        case 0:
            LOGE("Unknown option: %s", argv[optind]);
            exit(-1);
            break;
        case 'h':
            usage(argv[0]);
    ...
    if (state->user_dmesg) {
        ...
        if (user_dmesg_fs < 0) {
            LOGE("can't open %s for append", state->user_dmesg);
        }
    ...
    for (int run_no = 0; 1; run_no += 1) {
        ...
        /* CALL fuzzer function with corrected buffer */
        FUZZ_FUNCTION(buf, buf_len);
